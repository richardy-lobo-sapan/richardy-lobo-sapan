...
**AI & LLM**
`Python` `Scikit-learn` `TensorFlow` `Keras` `PyTorch` `OpenCV` `Groq API` `Anthropic API` `Llama 3.1` `LSTM` `LangChain` `ChromaDB` `RAG` `Gemini API` `LangGraph` `AI Agents` `ReAct` `Tavily`
